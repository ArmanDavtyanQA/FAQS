-- Topics + many-to-many links to FAQs (multiple topics per question)
-- RLS uses SECURITY DEFINER helpers to avoid infinite recursion between
-- faq_topics (public select scans links) and faq_topic_links (select scans faqs).
...
-- Helpers: bypass RLS inside body so policies on links/topics/faqs do not recurse
